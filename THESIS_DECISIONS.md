...
**Decision:** Confirmed by the IMT technical advisor — variants are generated by **dropping feature columns** entirely from the input pipeline, not by replacing values with mean/median.
...
- **Value-based masking variants.** Per the IMT technical advisor's confirmation, all variant generation is column dropping. Masking was considered as an alternative protocol and explicitly rejected in favor of dropping for the reasons in §6 and §7.C.
